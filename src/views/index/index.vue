    ...
    <a-layout-sider :collapsed="store.state.collapsed"
      ...
      <a-menu v-model:selectedKeys="data.selectedKeys" mode="inline" :open-keys="data.openKeys"
        ...
        <a-sub-menu v-for="item in hasChildren" :key="item.lable">
          ...
          <template v-else-if="`${item.lable}` === '图库列表'" #icon>
            <tags-two-tone />
          </template>
          <template v-else-if="`${item.lable}` === '视频检测'" #icon>
            <tags-two-tone />
          </template>
...
import { FolderOpenTwoTone, TagsTwoTone, RightOutlined, LeftOutlined } from '@ant-design/icons-vue'
  ...
  setup () {
    ...
    const menuList = [{
      ...
      children: [{
        ...
        lable: '视频检测',
        path: '/albumAdmin/videoDetection'
      }, {
        lable: '接口使用',
