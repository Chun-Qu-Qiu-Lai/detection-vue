<template>
  <a-spin tip="检测中..." size="large" :spinning="data.spinning">
    <div style="display: flex;align-items: center;justify-content: space-around;padding: 20px 20px;"><a-button
        type="primary" @click="updataModelVisible()"
        style="border-radius: 1.25rem;background: rgb(255,151,0);border: none;">新建图库</a-button>
    </div>
    <a-table :columns="columns" :data-source="data.AblbumTables" class="components-table-demo-nested"
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'operation'">
          <a-popconfirm v-if="data.AblbumTables.length" title="会删除图库内所有图片" ok-text="确认" cancel-text="取消"
            @confirm="deleteAblbum(record.catalogueId)">
            <a-button type="primary" danger
              style="background: linear-gradient(0deg, rgba(251,33,117,1) 0%, rgba(234,76,137,1) 100%);border-radius: 1.25rem;">删除</a-button>
          </a-popconfirm>
          <a-divider type="vertical" />
          <a-button type="primary" @click="openUpdateAlbumNameModel(record.catalogueId)"
            style="border-radius: 1.25rem;">修改</a-button>
          <a-divider type="vertical" />
          <a-upload @change="handleChange" v-model:file-list="data.fileList" name="file" :action="data.host"
            ...
            <a-button type="primary" @click="updateUploadCatalogue(record.catalogueId)" style="border-radius: 1.25rem;">
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'operation'">
              <span class="table-operation">
                ...
                <a-divider type="vertical" />
                <a class="ant-dropdown-link" @click="detection(record.fileUrl)">检测</a>
                <a-divider type="vertical" />
  ...
  <a-modal :destroyOnClose="true" width="400px" v-model:visible="data.albumNameModelVisible" @ok="changeAlbumName()"
    :closable="false" ok-text="修改" cancel-text="取消" @cancel="cancelUpdataName()">
    <a-form>
  ...
  <a-modal v-model:visible="data.displayVisible" title="检测结果" ok-text="确认" cancel-text="取消"
    :bodyStyle="{ padding: '5px 10px', width: '900px' }" :width="900">
    <div style="display: flex;width: 900px;justify-content: space-around;">
      <img style="width: 400px;height: 400px;" :src="data.sUrl" />
      <img style="width: 400px;height: 400px;" :src="data.dUrl" />
    </div>
  </a-modal>
</template>
<script>
...
export default defineComponent({
  ...
  setup () {
    ...
    const beforeUpload = async (file) => {
      ...
      console.log(data.signaturesignatureFormData)
    ...
    // 图片检测
    const detection = async (fileUrl) => {
      data.spinning = true
      // setTimeout(() => {
      //   message.success('检测成功')
      //   data.displayVisible = true
      //   data.spinning = false
      // }, 5000)
      const { data: res } = await picDetect(fileUrl)
      if (res.code === 200) {
        message.success('检测成功')
        data.sUrl = fileUrl
        data.spinning = false
